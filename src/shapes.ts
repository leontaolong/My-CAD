...
/**
 * Represents a shape that can be drawn (for graphical views)
 */
export interface DrawableShape extends Shape { //use Shape class as interface
  /**
   * How to draw on a graphical view
   * @param brush the graphics context to draw on
   */
  draw(brush:CanvasRenderingContext2D):void;
}
 ...
 */
export class Rectangle extends Shape implements DrawableShape {
 ...
 * Represents a circle
 */
export class Circle extends Shape implements DrawableShape {
 ...
 */
export class Triangle extends Shape implements DrawableShape {
  ...
  private static area(x1, y1, x2, y2, x3, y3):number{
